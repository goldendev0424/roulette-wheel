function Observable() {
  ...
  this.withLatestFrom = right => source =>
    new Observable(observer => {
      var right_val = null;
  ...
  this.timer = (interval, limit = Infinity) =>
    new Observable(observer => {
      var i = 0;
      const t = setInterval(() => {
        observer.next(i++);
  ...
  this.fromEvent = (element, eventname) =>
    new Observable(observer => {
  ...
  this.pipe = (initialValue, ...fns) => {
    return fns.reduce((state, fn) => fn(state), initialValue);
  };

  this.takeUntil = notifier => source =>
    new Observable(observer => {
